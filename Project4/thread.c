               ...
               thread_func *function, void *aux)
{
  ...
  /* Set default exit status in case the new thread fails to set it explicitly. */
  t->exit_status = TID_ERROR;

  /* Set the parent of the new thread. */
  t->parent = curr;

  /* Allocate and initialize a new child struct to track this child process. */
  struct child *child_item = malloc(sizeof(struct child));
  
  child_item->tid = tid;
  child_item->is_waiting = false;
  child_item->has_exited = false;
  child_item->load_success = false;
  child_item->exit_status = TID_ERROR;
  sema_init(&child_item->exec_sync, 0);

  /* Add the child to the parent's list of children. */
  list_push_back(&curr->children, &child_item->elem);
  #endif
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  // since it may create child processes later.
  // This is done here instead of in thread_create(),
  // as this code runs for every thread at the very beginning.
